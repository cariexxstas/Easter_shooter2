...
# фонова музика
mixer.init()
mixer.music.load('222.mp3')
mixer.music.play()
fire_sound = mixer.Sound('fire.ogg')
 
# нам потрібні такі картинки:
img_back = "fotofon.jpg"  # фон гри
img_hero = "koschik.png"  # герой
img_bullet = "bullet.png" # куля
img_enemy = "eggs.png"  # ворог
img_ast = "banana.png" # астероїд
...
for i in range(1, 3):
    monster = Enemy(img_enemy, randint(
        80, win_width - 80), -40, 40, 50, randint(1, 5))
...
while run:
    ...
    if not finish:
        ...
        if sprite.spritecollide(ship, monsters, False):
            ...
            score = score + 1
            monster = Enemy(img_enemy, randint(80, win_width - 80), -40, 40, 50, randint(1, 101))
            monsters.add(monster)
    ...
    #бонус: автоматичний перезапуск гри
        ...
        time.delay(3000)
        for i in range(1, 3):
            monster = Enemy(img_enemy, randint(80, win_width - 80), -40, 40, 50, randint(1, 5))
            monsters.add(monster)
        for i in range(1, 3):
            asteroid = Aster(img_ast, randint(30, win_width - 30), -40, 80, 50, randint(1, 7))
            asteroids.add(asteroid)    
